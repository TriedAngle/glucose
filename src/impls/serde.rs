...
impl<'de, T: Num + Default + Deserialize<'de> + Copy, const N: usize> Deserialize<'de> for Vector<T, { N }> {
...
impl<'de, T: Num + Default + Deserialize<'de> + Copy, const N: usize> Visitor<'de> for VectorVisitor<T, { N }> {
    ...
    fn visit_seq<V>(self, mut seq: V) -> Result<Vector<T, { N }>, V::Error> where V: SeqAccess<'de> {
